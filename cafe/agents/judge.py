...
class JudgeAgent(BaseAgent):
    ...
    def run(self, sql_query: str, business_question: str) -> bool:
        ...
        is_useful = self._assess_usefulness(sql_query, business_question, sql_result=sql_result)
        if is_useful:
            self.logger.info(f"Generated query is valid and useful: {sql_query}")
            return True
        else:
            self.logger.warning(f"Generated query is valid but not deemed useful: {sql_query}")
            return False
    ...
    def _assess_usefulness(self, sql_query: str, business_question: str, sql_result: dict[str, Any], use_heuristic=False) -> bool:
        """Assesses the usefulness of the query based on its structure and results."""
        # Option 1: Simple heuristic analysis of the query structure
        if use_heuristic:
            return self._heuristic_analysis(sql_query, business_question)

        # Option 2: Using the Snowflake LLM API for more sophisticated analysis
        # This would involve calling the 'complete' endpoint with a prompt that asks
        # the LLM to evaluate the query and its potential impact based on the
        # original prompt.
...
---
in the context of the following user prompt:
{business_question}
---
Considering the goal of extending the database for future engineering tasks,
does this query seem likely to contribute meaningfully? Explain your reasoning
and return '<EVAL>1</EVAL>' if contributes meaningfully '<EVAL>0</EVAL>' if not in final line.
        """
        ...
        if "<EVAL>1</EVAL>" in response_content:
            return True
        else:
            return False
